...
class CakeFileMakerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("CAKE File Maker 0.0.1")
    ...
    def create_widgets(self):
        ...
        self.button_image_files = tk.Button(self.root, text="Browse", command=self.select_image_files)
        ...
        self.listbox_image_files.grid(row=1, column=0, columnspan=2, padx=10, pady=5)
        ...
        self.text_poni_file.grid(row=3, column=0, columnspan=2, padx=10, pady=5)
        ...
        self.check_overwrite.grid(row=4, column=0, columnspan=2, padx=10, pady=5)

        # Button to make cakes
        self.button_make_cakes = tk.Button(self.root, text="Make Cakes", command=self.make_cakes)
        self.button_make_cakes.grid(row=5, column=0, columnspan=2, padx=10, pady=10)
    ...
    def select_poni_file(self):
        ...
        if file_path:
            self.poni_filename = file_path
            self.text_poni_file.delete(0, tk.END)  # Clear the textbox before inserting new text
            self.text_poni_file.insert(0, file_path)  # Display the PONI file name in the textbox
